"""Run LongVideoBench samples with the Aria model using INT4 quantization."""
...
MODEL_ID = os.getenv("ARIA_INT4_MODEL_ID", "rhymes-ai/Aria-sequential_mlp")
DEVICE_MAP = os.getenv("ARIA_INT4_DEVICE_MAP", "auto")
MAX_FRAMES = int(os.getenv("MAX_FRAMES", "64"))
NUM_SAMPLES = int(os.getenv("NUM_SAMPLES", "9999"))
RESULT_LOG_PATH = os.getenv(
    "ARIA_INT4_RESULT_LOG", os.path.join("output", "aria_int4_video_samples.json")
)
...
def _run_sample(model, processor, sample: Dict[str, Any]) -> str:
    ...
    if "pixel_values" in inputs:
        # Auto-match dtype of the vision tower (usually float16 when quantized)
